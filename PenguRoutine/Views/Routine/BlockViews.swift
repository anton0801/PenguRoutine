...
struct PenguRoutineWebView: View {
    ...
    private func initialize() {
        let temp = UserDefaults.standard.string(forKey: GlacierKey.pushURL)
        let stored = UserDefaults.standard.string(forKey: GlacierKey.floesURL) ?? ""
        targetURL = temp ?? stored
        isActive = true
        if temp != nil { UserDefaults.standard.removeObject(forKey: GlacierKey.pushURL) }
    }
    
    private func reload() {
        if let temp = UserDefaults.standard.string(forKey: GlacierKey.pushURL), !temp.isEmpty {
            isActive = false
            targetURL = temp
